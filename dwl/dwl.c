...
{
    Client *c;
    wl_list_for_each(c, &clients, link) if (c->mon == m) wlr_scene_node_set_enabled(&c->scene->node, VISIBLEON(c, m));
...
void closemon(Monitor *m)
{
    ...
    wl_list_for_each(c, &clients, link)
    {
        if (c->isfloating && c->geom.x > m->m.width)
            resize(c, (struct wlr_box){.x = c->geom.x - m->w.width, .y = c->geom.y, .width = c->geom.width, .height = c->geom.height}, 0);
        if (c->mon == m)
...
void commitlayersurfacenotify(struct wl_listener *listener, void *data)
{
    ...
    if (wlr_layer_surface->current.committed == 0 && layersurface->mapped == wlr_layer_surface->mapped)
        return;
...
void createkeyboard(struct wlr_keyboard *keyboard)
{
    ...
    kb->key_repeat_source = wl_event_loop_add_timer(wl_display_get_event_loop(dpy), keyrepeat, kb);
...
void createpointer(struct wlr_pointer *pointer)
{
    ...
    {
        struct libinput_device *libinput_device = (struct libinput_device *)wlr_libinput_get_device_handle(&pointer->base);
...
Monitor *dirtomon(enum wlr_direction dir)
{
    ...
    if (!wlr_output_layout_get(output_layout, selmon->wlr_output))
        return selmon;
    if ((next = wlr_output_layout_adjacent_output(output_layout, dir, selmon->wlr_output, selmon->m.x, selmon->m.y)))
        return next->data;
    if ((next = wlr_output_layout_farthest_output(output_layout, dir ^ (WLR_DIRECTION_LEFT | WLR_DIRECTION_RIGHT), selmon->wlr_output, selmon->m.x, selmon->m.y)))
        return next->data;
    return selmon;
}
...
void handlesig(int signo)
{
    if (signo == SIGCHLD)
    {
         ...
         * XWayland.
         */
        while (!waitid(P_ALL, 0, &in, WEXITED | WNOHANG | WNOWAIT) && in.si_pid && (!xwayland || in.si_pid != xwayland->server->pid))
...
void mapnotify(struct wl_listener *listener, void *data)
{
    ...
    if (client_is_unmanaged(c))
    {
        ...
        wlr_scene_node_set_position(&c->scene->node, c->geom.x + borderpx, c->geom.y + borderpx);
...
unset_fullscreen:
    ...
    wl_list_for_each(w, &clients, link) if (w != c && w->isfullscreen && m == w->mon && (w->tags & c->tags)) setfullscreen(w, 0);
}
...
void motionnotify(uint32_t time)
{
    ...
    if (cursor_mode == CurMove)
    {
        ...
        resize(grabc, (struct wlr_box){.x = cursor->x - grabcx, .y = cursor->y - grabcy, .width = grabc->geom.width, .height = grabc->geom.height}, 1);
        return;
    }
    else if (cursor_mode == CurResize)
    {
        resize(grabc, (struct wlr_box){.x = grabc->geom.x, .y = grabc->geom.y, .width = cursor->x - grabc->geom.x, .height = cursor->y - grabc->geom.y}, 1);
        return;
    }
    ...
    {
        if ((type = toplevel_from_wlr_surface(seat->pointer_state.focused_surface, &w, &l)) >= 0)
        {
            c = w;
...
void moveresize(const Arg *arg)
{
    ...
    case CurResize:
        ...
        wlr_xcursor_manager_set_cursor_image(cursor_mgr, (cursor_image = "bottom_right_corner"), cursor);
        break;
    }
}
...
void outputmgrapplyortest(struct wlr_output_configuration_v1 *config, int test)
{
    ...
    wl_list_for_each(config_head, &config->heads, link)
    {
        ...
        if (config_head->state.mode)
            ...
            wlr_output_set_custom_mode(wlr_output, config_head->state.custom_mode.width, config_head->state.custom_mode.height, config_head->state.custom_mode.refresh);

        /* Don't move monitors if position wouldn't change, this to avoid
         * wlroots marking the output as manually configured */
        if (m->m.x != config_head->state.x || m->m.y != config_head->state.y)
            wlr_output_layout_move(output_layout, wlr_output, config_head->state.x, config_head->state.y);
...
{
    c->isfloating = floating; // floating 参数为当前聚焦窗口的浮动属性相反的值
    if (!c->mon)
        return;
    // 把当前节点也就是当前窗口从当前场景删除并添加到新的场景(float 或 tile)
    wlr_scene_node_reparent(&c->scene->node, layers[c->isfloating ? LyrFloat : LyrTile]);
    arrange(c->mon); // 重排当前屏幕的窗口
    printstatus();   // 更新状态栏
}
...
void setfullscreen(Client *c, int fullscreen)
{
    ...
    wlr_scene_node_reparent(&c->scene->node, layers[fullscreen ? LyrFS : c->isfloating ? LyrFloat : LyrTile]);
...
void setlayout(const Arg *arg)
{
    if (!selmon)
        return;
    // 参数为空或传入了参数并且参数指定布局和当前屏幕的布局不一样
    if (!arg || !arg->v || arg->v != selmon->lt[selmon->sellt])
        selmon->sellt ^= 1; // 选择和当前布局不一样的布局
    // 参数不为空
    if (arg && arg->v)
        selmon->lt[selmon->sellt] = (Layout *)arg->v; // 设置当前屏幕布局为参数指定的布局
    strncpy(selmon->ltsymbol, selmon->lt[selmon->sellt]->symbol, LENGTH(selmon->ltsymbol));
    arrange(selmon); // 重排指定屏幕
    printstatus();   // 更新状态栏
}
...
void setup(void)
{
    ...
    locked_bg = wlr_scene_rect_create(layers[LyrBlock], sgeom.width, sgeom.height, (float[4]){0.1, 0.1, 0.1, 1.0});
    wlr_scene_node_set_enabled(&locked_bg->node, 0);
...
void tile(Monitor *m)
{
    ...
    wl_list_for_each(c, &clients, link) if (VISIBLEON(c, m) && !c->isfloating && !c->isfullscreen) n++;
    if (n == 0)
        return;
...
{
    Client *sel = focustop(selmon); // 获取当前聚焦窗口
    /* return if fullscreen */
    if (sel && !sel->isfullscreen)          // 当前有聚焦的窗口并且不是全屏窗口
        setfloating(sel, !sel->isfloating); // 设置当前聚焦窗口的浮动状态和之前相反
}
...
void unmaplayersurfacenotify(struct wl_listener *listener, void *data)
{
    ...
    if (layersurface->layer_surface->output && (layersurface->mon = layersurface->layer_surface->output->data))
        arrangelayers(layersurface->mon);
    if (layersurface->layer_surface->surface == seat->keyboard_state.focused_surface)
        focusclient(focustop(selmon), 1);
    motionnotify(0);
}
...
void updatemons(struct wl_listener *listener, void *data)
{
    ...
    wl_list_for_each(m, &mons, link) if (m->wlr_output->enabled && !wlr_output_layout_get(output_layout, m->wlr_output)) wlr_output_layout_add_auto(output_layout, m->wlr_output);
    ...
    {
        wl_list_for_each(c, &clients, link) if (!c->mon && client_is_mapped(c)) setmon(c, selmon, c->tags);
        focusclient(focustop(selmon), 1);
...
void configurex11(struct wl_listener *listener, void *data)
{
    ...
    if (c->isfloating || c->type == X11Unmanaged)
        resize(c, (struct wlr_box){.x = event->x, .y = event->y, .width = event->width, .height = event->height}, 0);
    else
        arrange(c->mon);
}
...
void xwaylandready(struct wl_listener *listener, void *data)
{
    ...
    if ((xcursor = wlr_xcursor_manager_get_xcursor(cursor_mgr, "left_ptr", 1)))
        wlr_xwayland_set_cursor(xwayland, xcursor->images[0]->buffer, xcursor->images[0]->width * 4, xcursor->images[0]->width, xcursor->images[0]->height, xcursor->images[0]->hotspot_x, xcursor->images[0]->hotspot_y);

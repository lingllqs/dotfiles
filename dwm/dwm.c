...
void buttonpress(XEvent *e)
{
    ...
    if (ev->window == selmon->barwin || (!c && selmon->showbar && (topbar ? ev->y <= selmon->wy : ev->y >= selmon->wy + selmon->wh)))
    { // 点击在bar上
        i = x = 0;
...
void resizeclient(Client *c, int x, int y, int w, int h)
{
    ...
    if (((nexttiled(c->mon->clients) == c && !nexttiled(c->next))) && !c->isfullscreen && !c->isfloating)
    {
        c->w = wc.width += c->bw * 2;
...
{
    if (selmon->sel != NULL && __builtin_popcount(selmon->tagset[selmon->seltags] & TAGMASK) == 1 && selmon->tagset[selmon->seltags] > 1)
    {
...
{
    if (selmon->sel != NULL && __builtin_popcount(selmon->tagset[selmon->seltags] & TAGMASK) == 1 && selmon->tagset[selmon->seltags] & (TAGMASK >> 1))
    {
        tag(&(Arg){.ui = selmon->tagset[selmon->seltags] << 1});
    }
}
...
void togglefloating(const Arg *arg)
{
    ...
    if (selmon->sel->isfloating)
    {
        selmon->sel->x = selmon->wx + selmon->ww / 6, selmon->sel->y = selmon->wy + selmon->wh / 6, managefloating(selmon->sel);
...
void updatebars(void)
{
    unsigned int w;
    Monitor *m;
    XSetWindowAttributes wa = {.override_redirect = True, .background_pixel = 0, .border_pixel = 0, .colormap = cmap, .event_mask = ButtonPressMask | ExposureMask};
    ...
    for (m = mons; m; m = m->next)
    {
        ...
        if (showsystray && m == systraytomon(m))
            w -= getsystraywidth();
        m->barwin =
            XCreateWindow(dpy, root, m->wx, m->by, m->ww, bh, 0, depth, InputOutput, visual, CWOverrideRedirect | CWBackPixel | CWBorderPixel | CWColormap | CWEventMask, &wa);
...
void tile(Monitor *m)
{
    ...
    for (i = 0, my = sy = gappo, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
        if (i < m->nmaster)
        {
            resize(c, m->wx + gappo, m->wy + my, mw - 2 * c->bw - gappi, mh - 2 * c->bw, 0);
            my += HEIGHT(c) + gappi;
        }
        else
        {
            resize(c, m->wx + mw + gappo, m->wy + sy, m->ww - mw - 2 * c->bw - 2 * gappo, sh - 2 * c->bw, 0);
            sy += HEIGHT(c) + gappi;
        }
}
...
void exchange_two_client(Client *c1, Client *c2)
{
    ...
    if (tmp1->next == NULL)
    { /*  gDebug("tmp1->next==null"); */
        return;
    }
    if (tmp2->next == NULL)
    { /* gDebug("tmp2->next==null");  */
        return;
    }
